...
function Sparkle({ className }: { className: string }) {
  return (
    <svg viewBox="0 0 40 40" aria-hidden className={"pointer-events-none absolute text-gold " + className}>
      <path d="M20 0 C21.5 13 27 18.5 40 20 C27 21.5 21.5 27 20 40 C18.5 27 13 21.5 0 20 C13 18.5 18.5 13 20 0 Z" fill="currentColor" />
    </svg>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
        <span className="text-[5.5cqw] uppercase tracking-[0.22em] text-gold-light">{band.kicker}</span>
        <span className="mt-[1.5cqw] font-heading text-[17cqw] font-semibold leading-none text-[#fbf3dc]">{band.amount}</span>
...
export function PriceTiles({ bands }: { bands: PriceBand[] }) {
  ...
  return (
    <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#f5efdf] via-ivory to-[#e2ece6] px-5 py-10 sm:px-10 sm:py-14">
      <div aria-hidden className="pointer-events-none absolute -left-24 -top-24 h-72 w-72 rounded-full bg-gold/15 blur-3xl" />
      <div aria-hidden className="pointer-events-none absolute -bottom-28 -right-20 h-80 w-80 rounded-full bg-brand/10 blur-3xl" />
